...
def write_sky_file(output_path="./", limits={}):
    ...
    path_to_targets = '/global/cscratch1/sd/adamyers/dr9/0.54.0.dev4695/skies/'
...
def write_onepct_mtl(output_path="./", program='bright', survey='main', limits={}):
    ...
    # List all the fits files to read
    path_to_targets = '/global/cscratch1/sd/adamyers/dr9/0.54.0.dev4695/targets/' + survey +'/resolve/'+program+'/'
...
def write_onepct_secondary(output_path="./", program='bright', survey='main', limits={}):
    ...
    full_mtl_file = os.path.join(output_path, "secondary_{}_{}_onepct.fits".format(program, survey))
    ...
    # List all the fits files to read
    path_to_targets = '/global/cscratch1/sd/adamyers/dr9/0.54.0.dev4695/targets/' + survey +'/secondary/'+program+'/'
    target_files = glob.glob(os.path.join(path_to_targets, "*targets*fits"))
    print('target files to read:', len(target_files))
    target_files.sort()
    
    secondary_file = '/global/cscratch1/sd/adamyers/dr9/0.54.0.dev4695/targets/{}/secondary/{}/sv3targets-{}-secondary.fits'.format(survey,program, program)
    # Read the first file, only the columns that are useful for MTL
...
mtl_file_bright = write_onepct_mtl(output_path='./inputs/', program='bright', survey='sv3', limits=limits)
secondary_file_bright = write_onepct_secondary(output_path='./inputs/', program='bright', survey='sv3', limits=limits)
updated_mtl_file_bright = update_bgs(mtl_file_bright)

mtl_file_dark = write_onepct_mtl(output_path='./inputs/', program='dark', survey='sv3', limits=limits)
