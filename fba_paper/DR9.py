...
def cut_mtl_sky_tiles(targets_path="./", tile_path="./", cut_name="cut", limits={}, hemisphere='north'):
    ...
    try:
        if not os.path.exists(cut_sky_file):
            north_sky_file = os.path.join(targets_path, "sky_{}.fits".format(hemisphere))
...
def write_initial_mtl_files(output_path="./", program='bright'):
    ...
    full_mtl_file = os.path.join(output_path, "{}_full.fits".format(program))
    
    if os.path.exists(north_mtl_file) and os.path.exists(south_mtl_file) and os.path.exists(full_mtl_file):
        print("Files {} {} {} already exist".format(north_mtl_file, south_mtl_file, full_mtl_file))
        return

    # List all the fits files to read
    path_to_targets = '/global/cfs/cdirs/desi/target/catalogs/dr9m/0.44.0/targets/main/resolve/'+program+'/'
